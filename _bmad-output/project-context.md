...
recent_updates: 'Added critical warnings about NOT editing root index.html (build artifact) - agents must edit source files in /src/ instead'
---
...
**Build & Deployment Workflow (Option B Architecture):**
- **Development:** Vite dev server (localhost:5173) with hot module replacement
- **Build:** `npm run build` builds to `/dist/` then copies `index.html` to project root
- **Deployment:** Commit built artifact + source to git, Pi auto-pulls
- **Build Output:** Single HTML file with inlined CSS/JS at root (no external dependencies on Pi)
- **Temporary artifacts:** `/dist/` folder (gitignored, cleaned on each build)
- **Architecture rationale:** Balances Vite best practices (clean dist/ builds) with Pi deployment simplicity (no build tools needed)
...
### ⚠️ FILE EDITING RULES - READ FIRST ⚠️

**NEVER EDIT BUILD ARTIFACTS:**
- ❌ **DO NOT EDIT `/index.html` at project root** - This is a GENERATED FILE from Vite build
- ✅ **ALWAYS EDIT `/src/index.html`** - This is the source file for HTML structure
- ✅ **EDIT JavaScript in `/src/js/*.js`** - Source modules
- ✅ **EDIT CSS in `/src/css/*.css`** - Source stylesheets
- 🔨 **After editing source files, run `npm run build`** to regenerate `/index.html`

**Source Files (EDIT THESE):**
```
/src/index.html          ← Edit HTML structure here
/src/js/main.js          ← Edit main application logic
/src/js/carousel-controller.js
/src/js/item-highlighter.js
/src/js/detail-panel.js
...
```
/index.html              ← Build artifact - DO NOT EDIT DIRECTLY
/dist/                   ← Temporary build folder - DO NOT EDIT
```

### Language-Specific Rules (JavaScript/ES Modules)

**Multi-File Architecture (CURRENT - Option B):**
- **Source structure:** `/src/js/` for JavaScript modules, `/src/css/` for stylesheets, `/src/index.html` for HTML
- **Build artifacts:** `/dist/` folder (temporary, gitignored, cleaned on each build)
- **Build output:** Single `index.html` at project root (copied from `dist/src/index.html`) - **DO NOT EDIT THIS FILE**
- **Component separation:** carousel-controller.js, item-highlighter.js, detail-panel.js, api-client.js, utils.js
- **Deployment:** Pi serves root `index.html` (no build tools needed on Pi)
...
# Build for deployment (Option B: dist/ + copy to root)
npm run build
# Step 1: Vite builds to /dist/ folder
# Step 2: Script copies dist/src/index.html to project root
# Result: index.html at root ready for Pi deployment

# Optional: Test built artifact locally
python3 -m http.server 8000  # Verify production build works
```

**Vite Build Configuration (Option B):**
- Output directory: `/dist/` (temporary, gitignored)
- Post-build copy: `dist/src/index.html` → `index.html` (root)
...
- Architecture: Vite best practice (separate build folder) + Pi simplicity (root index.html)

**⚠️ CRITICAL: Root index.html is GENERATED - DO NOT EDIT**
- The `/index.html` at project root is the OUTPUT of `npm run build`
- Any manual edits to root `/index.html` will be OVERWRITTEN on next build
- ALWAYS edit `/src/index.html` for HTML changes
- ALWAYS edit `/src/js/*.js` for JavaScript changes
- ALWAYS edit `/src/css/*.css` for CSS changes
- After editing source files, run `npm run build` to regenerate root index.html
...
- ❌ Don't edit `/index.html` at project root directly (it's a generated build artifact)
...
- ❌ Don't make changes in `/dist/` folder (temporary build output, gets cleaned)
...
**❌ NEVER edit build artifacts:**
- Don't edit `/index.html` at project root (it's generated by Vite build)
- Don't edit files in `/dist/` folder (temporary build output)
- Don't make changes without rebuilding (`npm run build` after editing source)
- Don't commit source changes without the corresponding built artifact
- **ALWAYS edit source files in `/src/` directory instead**
...
1. **NEVER edit /index.html at root:** It's a build artifact - ALWAYS edit /src/index.html instead
2. **Build before push:** `npm run build` THEN `git push` - never skip the build step
3. **Edit source, not artifacts:** All code changes go in /src/ directory, never in /dist/ or root index.html
4. **Preserve utility functions:** Always reuse formatDate(), stripHtml(), truncate() from utils.js
5. **Test on actual TV:** Desktop browser testing insufficient - verify at viewing distance on Pi
6. **Match GitHub exactly:** When in doubt, inspect GitHub.com and copy their approach
7. **Timer cleanup:** Memory leaks will crash 24/7 kiosk - always clearInterval() in stop()
8. **Chromium 84 compatibility:** Check caniuse.com for Chromium 84 support before using features
9. **Component wiring:** Wire callbacks in main.js - don't forget onPageChange, onItemChange
10. **Cache fallbacks:** Return stale cache during errors - something is better than nothing
11. **ES module extensions:** Always include .js extension in import paths
12. **Item timer reset:** When page changes, item timer MUST reset - critical for carousel sync
